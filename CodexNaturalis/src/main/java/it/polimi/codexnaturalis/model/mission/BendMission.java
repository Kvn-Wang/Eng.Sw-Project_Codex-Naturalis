                    ...
                    for (int j = 2; j < (mapArray[0].length); j++) {
                        if (mapArray[i][j] != null) {
                            if (mapArray[i][j].getColor() == decorationResource && !usedCardArray[i][j]) {
                                if (mapArray[i][j - 1] != null && mapArray[i][j - 1].getColor() == pillarResource && mapArray[i + 1][j - 2] != null &&  mapArray[i + 1][j - 2].getColor() == pillarResource) {
                                    match++;
            ...
            case NORTH:
                for (int i = 0; i < (mapArray.length - 2); i++) {
                    for (int j = 1; j < (mapArray[0].length); j++) {
                        if (mapArray[i][j] != null) {
                            if (mapArray[i][j].getColor() == decorationResource && !usedCardArray[i][j]) {
                                if (mapArray[i+1][j] != null && mapArray[i + 1][j].getColor() == pillarResource && mapArray[i+2][j-1] != null && mapArray[i + 2][j - 1].getColor() == pillarResource) {
                                    match++;
                                    usedCardArray[i][j] = true;
